...
const dir = 'C:/Users/rampr/OneDrive/Desktop/B2B_Fintech/src/admin/components/MemberPages';
...
files.forEach(file => {
    ...
    // Replace the export buttons wrapper (could be styles.exportRow or inline flex)
    // We look for a div that contains FaFileExcel and FaPrint etc.
    ...
    const exportRegex3 = /<div className=\{styles\.actionRow\}>(\s*<button[^>]*>[\s\S]*?FaCopy[\s\S]*?)<\/div>/;
    ...
    if (exportRegex1.test(content)) {
        ...
        // Fallback: replace any div containing FaCopy and FaFileExcel
    ...
    // Replace Empty State
    // "No members found", "No data available", "No records found" -> <FiDatabase /> ...
