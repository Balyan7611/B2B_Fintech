...
import { 
  FiSearch, FiEdit, FiTrash2, FiPlus, FiChevronLeft, FiChevronRight, FiChevronDown, FiDatabase, FiX, FiCheck, FiUserCheck, FiSettings, FiGrid, FiArrowRight, FiRefreshCw, FiInfo
} from 'react-icons/fi';
      ...
      {isOpen && (
        <div style={{ 
          ...
          <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
             ...
             {filtered.length === 0 && <div style={{ padding: '15px', color: '#94A3B8', fontSize: '0.85rem', textAlign: 'center' }}>No results found</div>}
          </div>
        </div>
      )}
    </div>
...
const AssignService = () => {
  ...
  // Member Username Options
  const memberOptions = [
    "VISHNU123 (Vishnu Sharma)", 
    "RAM456 (Ram Prasad)", 
    "MDT8597 (Vivek Varshney)", 
    "Pay99RT4011 (Gaurav Kumar)", 
    "Pay99RT4014 (Pooja Tomar)"
  ];
           ...
           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px', alignItems: 'flex-end', marginBottom: '30px' }}>
              <div className={styles.formGroup} style={{ margin: 0 }}>
                 <label className={styles.label} style={{ fontWeight: 700, color: '#4E6080', display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '8px' }}>
                   <FiUserCheck style={{ color: '#1756AA' }} /> Member ID / Username *
                 </label>
                 <CustomSearchSelect 
                   options={memberOptions} 
                   placeholder="Select Member ID / Username" 
                   value={memberId} 
